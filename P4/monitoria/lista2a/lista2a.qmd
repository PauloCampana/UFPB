...
Lembre da distribuição acumulada da exponencial: $F(x) = 1 - e^{-\lambda x}$
...
$$
\p{X < 70 | X \geq 50} = \p{X < 20} = F(20) = 1 - e^{-\frac{1}{60}20} = 1 - e^{-\frac{1}{3}} = `r pexp(20, rate = 1/60) |> round(3)`
$$
...
Agora a probabilida de que uma lâmpada selecionada ao acaso dure pelo menos 90 horas:
...
$Y \sim \text{Bin}(n = 10, \; p = 0.349)$

$$
P(Y = 6) = \binom{10}{6} \; 0.349^6 \; 0.651^4 = `r dbinom(6, size = 10, prob = 0.349) |> round(3)`
$$
...
$$
\dfrac{1}{a} \int \limits_{-\infty}^{\infty} \dfrac{1}{\sqrt{2 \pi} \sigma} \text{exp} \left \lbrace -\dfrac{1}{2} \left( \dfrac{\frac{y -b}{a} - \mu}{\sigma} \right) \right \rbrace dx = 
\int \limits_{-\infty}^{\infty} \dfrac{1}{\sqrt{2 \pi} (a \sigma)} \text{exp} \left \lbrace -\dfrac{1}{2} \left( \dfrac{y - (a \mu + b)}{(a \sigma)} \right) \right \rbrace dx
$$
...
\begin{align*}
    \Var{x} &= \sigma^2                                   \\
    \Var{x} &= \Var{a X + b} = a^2 \Var{X} = a^2 \sigma^2
\end{align*}
