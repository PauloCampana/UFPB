...
## RESUMO
Neste relatório será apresentado uma análise em relação os índices demográficos e a população em geral do Estado do Rio de Janeiro, em específico sobre os índices de natalidade, mortalidade e taxa de crescimento populacional do período 2011 a 2020 e projeção da população do Estado para até 2030, com base nas contagens dos censos de 2000 e 2010. será explicado a metodologia e o passo a passo da obtenção dos índices e projeções com base nos conceitos vistos em aula e os materiais de referência @paes e slides da aula. Será abordado três métodos de projeção populacional: os modelos aritiméticos, geométricos e logísticos, suas comparações com as previsões do IBGE de 2018 para toda a população e separadamente para homens e mulheres. Os dados de população utilizados para esta análise foram obtidos da projeção de 2018 encontradas no site do IBGE e as informações sobre natalidade e mortalidade proveem do sistema TABNET do DATASUS, que organiza dados do Sistema de Informações sobre Nascidos Vivos e Sistema de Informações sobre Mortalidade. Por fim os resultados da pesquisa são apresentados por meio de gráficos e tabelas e discutimos sobre oque as tais estatísticas apresentadas no trabalho irão significar para o futuro da composição demográfica do Rio de Janeiro e seu impacto nas demais áreas.
...
A composição deste documento foi feita com uma mistura de "Markdown" e \LaTeX, as tabelas e gráficos foram criadas atravez da linguagem `R` e apresentadas pelo software livre de publicação científica "Quarto" de forma que todo conteúdo do relatório seja reproduzível.
...
Com a @tbl-estrutura, vemos que no período 2011-2020, houve um acréscimo de 771.046 habitantes, um aumento de cerca de 4,7% da população, o número de nascimentos foi um pouco menos que o dobro do número de óbitos e as Taxas de Crescimento Natural e Geométrico são similares, indicando um crescimento bastante linear.
...
```{r}
#| label: tbl-proj
#| tbl-cap: Projeção para populacional total no Estado do Rio de Janeiro usando os modelos aritimético, geométrico e logístico, junto da projeção do IBGE.
proj |> 
    select(
        Ano = ano,
...
#| fig-height: 7.5
#| fig-pos: h!
#| fig-cap: Projeção populacional no Estado do Rio de Janeiro usando os modelos aritimético, geométrico e logístico, junto da projeção do IBGE.
proj |> 
    pivot_longer(-ano) |> 
    separate_wider_delim(name, "_", names_sep = "_") |> 
    mutate(
        value = value / 1000,
        name_2 = factor(name_2, levels = c("arit", "geom", "logi", "IBGE"))
    ) |> 
    ...
    labs(
        x = "Tempo (ano)",
        y = "Projeção populacional (milhares de habitantes)",
        color = "Projeção"
    ) +
    theme_bw() +
