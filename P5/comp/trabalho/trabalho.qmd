...
Foram usados 5 conjuntos de dados para comparação, um conjunto de dados sintéticos e 4 incluidos em pacotes populares do R:

* Quatro variáveis aleatórias de distribuição Normal Multivariada com vetor de médias 0 e matriz de covariância com moderada correlação entre as variáveis, e adicionado uma variável $y$ como combinação linear das outras 4 mais um erro normal.
* `iris` [@iris].
* `diamonds` [@diamonds].
* `penguins` [@penguins].
* `concrete` [@concrete].
