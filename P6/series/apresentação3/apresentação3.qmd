...
## Naïve
O método Naïve é um dos métodos de previsão mais simples disponíveis. Sua lógica é direta: a previsão de um passo à frente é igual ao valor mais recente observado, representado pela equação:

$$
\hat y_t = y_{t - 1}
$$

Embora essa abordagem possa parecer simplista, há situações em que esse método é surpreendentemente eficaz. Na previsão de temperatura por exemplo, a melhor estimativa para a temperatura daqui a cinco minutos é pelo método Naïve: a temperatura esperada será simplesmente a mesma que a atual.
...
## Naïve
Com esse método, os valores da raiz de erro quadrático e erro médio absoluto são relativamente altos, indicando que o método Naïve não está conseguindo prever os valores com grande precisão para o conjunto de dados. Isso é devido à simplicidade do método Naïve, que não leva em conta padrões de tendência ou sazonalidade nos dados.
...
## Naïve sazonal
Já este método leva em consideração a sazonalidade dos dados, é uma extensão do método Naïve básico, em dados sazonais, usar o último valor pode não ser a melhor abordagem, pois os padrões sazonais tem uma influência significativa nos dados. Resolvemos isso ao prever o próximo valor usando o valor do mesmo período de uma temporada anterior.
...
Por exemplo, para prever os valores de janeiro de um ano, ele usará os valores de janeiro do ano anterior.
